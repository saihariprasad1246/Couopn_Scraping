...
async function setUdemySpecificHeaders(page, headers) {
  delete headers["upgrade-insecure-requests"];//This header is against CORS policy so udemy's server has rejected the request.
...
async function getURLAndDaysLeft(threeDetails, headers){
    ...
    try {
      ...
      page.on("request", (request) => {
       if ( request.resourceType() === "image" || rejectRequestPattern.find((pattern) => request.url().match(pattern))) {
         request.abort();
       } else {
         request.continue();
         }
      });
      await page.goto(obj["href"], {waitUntil: "domcontentloaded"});
      await page.evaluate(async () => {
        const aTag = "a[class='btn_offer_block re_track_btn']";
        const articleTag = document.querySelector("div.rh-outer-wrap > div.rh-container > div.rh-content-wrap.clearfix > div.main-side.rh-post-wrapper.single.clearfix > article");
        const childDivs = Array.from(articleTag.children).filter(el => el.tagName === 'DIV');
        document.querySelector(aTag).setAttribute("target", "_self");
        document.querySelector(aTag).scrollIntoView({behavior: 'smooth', block: 'center', inline: 'center'});
        while(document.querySelector(aTag).getAttribute("href") === "javascript:void(0)") {
          childDivs.forEach(div => {
            div.scrollIntoView({ behavior: 'smooth', block: 'center', inline: 'center' });
            //console.log('Scrolled into view: ', div); // Optional: Logs each scrolled div
          });
      ...
      await setUdemySpecificHeaders(page, headers);
      await page.locator(anchorDOMPath, {visible : true}).click();
      ...
      if(await page.url() == "https://www.udemy.com/"){
        ...
        await page.close();
          ...
          await new Promise(resolve => setTimeout(resolve, 3000));//To complete all network requests to get required resources and also to find those two URLs.
        ...
        //console.log(err);
        if(obj["json-file's-url"] || obj["courseId"]) {
        ...
        }//Only when either of url or course id is available.
        else{
          console.log("Unable to obtain either url or it's course id :(");
        }
      } finally {
